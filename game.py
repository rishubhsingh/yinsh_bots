...
class Game:
    ...
    def updatePositions(self, positions, move, current_player):
        ...
        b = bsign
        flip = 1
        while (xring+a >= 0 and xring+a < rows and yring+b >= 0 and yring+b < rows and abs(positions[xring+a][yring+b]['piece'] != 2 and positions[xring+a][yring+b] != -1)):
            ...
            if (flip == 1 and abs(positions[xring+a][yring+b]['piece']) == 1):
    ...
    def get_max_length_created(self, positions, rows, rings, marker_val, ring_val, xring, yring, destx, desty, asign, bsign, m):
        ...
        correct_changes = 0
        max_len = 0
        while (xring+a >= 0 and xring+a < rows and yring+b >= 0 and yring+b < rows and positions[xring+a][yring+b] != -1):
            i = xring+a
            j = yring+b
